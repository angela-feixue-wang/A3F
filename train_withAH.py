...
from dataset import ImageDatasetFiveL, ImageDatasetFiveLSN, ImageDatasetFiveRGB, ImageDatasetFiveRGBSN
...
# (3) thop dummy variable (automatically changes based on model selection using if function)
...
logging.info(f"w_ana: {w_ana}, w_mag: {w_mag}, w_gain: {w_gain}, w_centering: {w_centering}, w_shadow: {w_shadow}")
logging.info(f"loss_func: {loss_func}")
logging.info(f"gamma_ana: {gamma_ana}, gamma_mag: {gamma_mag}, gamma_gain: {gamma_gain}, gamma_centering: {gamma_centering}, gamma_shadow: {gamma_shadow}\n")
# ===================================================================
# ===================================================================
# ===================================================================
...
def finetuning(model, model_name):
    if model_name == "resnet50":
        ...
        # Freeze everything
        for param in model.backbone.parameters():
            param.requires_grad = True # True unfreezes everything; False freezes everything
...
for epoch in range(total_epochs):
    ...
    with torch.no_grad():
        for images, label_ana, label_mag, label_gain, label_centering, label_shadow in val_loader:
            ...
            mask_mag = (label_mag != 0)
            mask_gain = (label_gain != 0)
            mask_centering = (label_centering != 0)
            mask_shadow = (label_shadow != 0)
            total_mag += mask_mag.sum().item()
            total_gain += mask_gain.sum().item()
            total_centering += mask_centering.sum().item()
            total_shadow += mask_shadow.sum().item()

            correct_ana += (pred_ana == label_ana).sum().item() # Counts how many predictions match the true labels
            correct_mag += (pred_mag[mask_mag] == label_mag[mask_mag]).sum().item()
            correct_gain += (pred_gain[mask_gain] == label_gain[mask_gain]).sum().item()
            correct_centering += (pred_centering[mask_centering] == label_centering[mask_centering]).sum().item()
            correct_shadow += (pred_shadow[mask_shadow] == label_shadow[mask_shadow]).sum().item()

    acc_ana = 100 * correct_ana / total # Calculate val accuracy
    acc_mag = 100 * correct_mag / total_mag
    acc_gain = 100 * correct_gain / total_gain
    acc_centering = 100 * correct_centering / total_centering
    acc_shadow = 100 * correct_shadow / total_shadow
    ...
    if val_loss_avg < best_val_loss:
        ...
        filename = f"ckpt/Run_{start_time}_{model_name}_{acc_ana:.0f}_{acc_mag:.0f}_{acc_gain:.0f}_{acc_centering:.0f}_{acc_shadow:.0f}.pth"
        torch.save({
            "epoch": epoch,
...
# Save final model
filename = f"ckpt/Run_{start_time}_{model_name}_{acc_ana:.0f}_{acc_mag:.0f}_{acc_gain:.0f}_{acc_centering:.0f}_{acc_shadow:.0f}_final.pth"
torch.save({
    "epoch": epoch,
    "model_state_dict": model.state_dict(),
    "optimizer_state_dict": optimizer.state_dict(),
