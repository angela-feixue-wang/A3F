...
from dataset import ImageDatasetEvalL, ImageDatasetEvalLSN, ImageDatasetEvalRGB
from models import SonoNet, FiveTaskWrapperSonoNet, FiveTaskWrapperResNet
...
if model_name in ["resnet50", "sononet"]:
    channel = 1
    dataset_class = ImageDatasetEvalLSN
...
def build_model_eval(model_name, num_ana=9, num_mag=3, num_gain=3, num_centering=5, num_shadow=1):
...
with torch.no_grad():
    for images, names in val_loader:
        ...
        # Forward pass
        logits_ana, logits_mag, logits_gain, logits_centering, logits_shadow = model(images)
        ...
        # Predictions
        pred_ana = logits_ana.argmax(dim=1)
        ...
        # Confidences
        conf_ana = torch.softmax(logits_ana, dim=1).max(dim=1)[0]
        ...
        sentence = sentence_generator.build_sentence(preds_dict)
        sen_pred_time_ms = (time.time() - sen_start_time) * 1000
        
        results.append({
            "frame": names[0], 
            "pred_ana": pred_ana[0].item(),
            "pred_mag": pred_mag[0].item(),
            "pred_gain": pred_gain[0].item(),
            "pred_centering": pred_centering[0].item(),
            "pred_shadow": pred_shadow[0].item(),
            "conf_ana": round(conf_ana[0].item(), 2),
            ...
            "sentence": sentence,
            "inf_time_ms": round(inference_time_ms, 3),
            "sen_pred_time": round(sen_pred_time_ms, 4),
        })
...
for name, (pred_col, gnd_col) in categories.items():
    ...
    if name == "Anatomy":
        ...
        df_filtered = full_results_df[full_results_df["gnd_ana"] != 8]
...
# Extract columns
pred_ana = full_results_df["pred_ana"]
gnd_ana = full_results_df["gnd_ana"]

# If gnd_ana = Other, don't include it in confusion matrices for mag, gain, centering, or shadow
df_filtered = full_results_df[full_results_df["gnd_ana"] != 8]
...
labels_ana = sorted(map_ana.keys())
display_labels_ana = [map_ana[i] for i in labels_ana]
...
cm_ana_og = confusion_matrix(gnd_ana, pred_ana, labels=labels_ana)
...
cm_ana_n = confusion_matrix(gnd_ana, pred_ana, labels=labels_ana, normalize="true") # normalize="true" will turn all values to [0,1]
...
def plot_and_save_confusion_matrices(
    cm_ana, cm_mag, cm_gain, cm_centering, cm_shadow,
    display_labels_ana, display_labels_mag, display_labels_gain,
    display_labels_centering, display_labels_shadow,
    output_svg_path
):
    cms = [cm_ana, cm_mag, cm_gain, cm_centering, cm_shadow]
    titles = ["Anatomy", "Magnification", "Gain", "Centering", "Shadow"]
    display_labels = [
        display_labels_ana,
        display_labels_mag,
        display_labels_gain,
...
plot_and_save_confusion_matrices(
    cm_ana_n, cm_mag_n, cm_gain_n, cm_centering_n, cm_shadow_n,
    display_labels_ana, display_labels_mag, display_labels_gain,
    display_labels_centering, display_labels_shadow,
    svg_path_norm
)
...
plot_and_save_confusion_matrices(
    cm_ana_og, cm_mag_og, cm_gain_og, cm_centering_og, cm_shadow_og,
    display_labels_ana, display_labels_mag, display_labels_gain,
    display_labels_centering, display_labels_shadow,
    svg_path_og
)
